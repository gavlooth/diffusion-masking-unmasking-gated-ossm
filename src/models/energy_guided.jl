# Energy-guided interleaving of OscMamba and windowed transformers
# ───────────────────────────────────────────────────────────────────────────────

# Mathematical definition:
# Given an input sequence X, first compute M = Mamba(X) and T = Transformer((X + M)/2).
# The fused hidden state is H = LayerNorm(M + T). Two affine heads yield
...
function initialparameters(rng::Random.AbstractRNG, block::EnergyInterleaveBlock)
    ...
    return (
        ...
        transformer = initialparameters(rngs[2], block.transformer),
...
function initialstates(rng::Random.AbstractRNG, block::EnergyInterleaveBlock)
    ...
    return (
        ...
        transformer = initialstates(rngs[2], block.transformer),
...
function (block::EnergyInterleaveBlock)(
    ...
    transformer_out, st_trans = block.transformer(attn_in, ps.transformer, st.transformer)
    fused = mamba_out + transformer_out
...
function build_energy_guided_llm(
    ...
    blocks = ntuple(
        _ -> begin
            ...
            transformer = WindowedTransformer(model_dim, num_heads, window_radius)
            EnergyInterleaveBlock(mamba, transformer, model_dim)
        end,
        num_layers,
    )
